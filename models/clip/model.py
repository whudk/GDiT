...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        return self.visual(self.preprocess(image).type(self.dtype))
...
class CLIPTextContextEncoder(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            checkpoint, _ = clip.load(pretrained, jit=False, device="cpu")
            checkpoint = checkpoint.float().state_dict()
            state_dict = {}
...
class cross_clip_sam(nn.Module):
    ...
    def forward(self, visual_clip, visual_sam):
...
class CLIPVisionTransformer(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        pretrained = pretrained or self.pretrained
        if isinstance(pretrained, str):
            self.clip_model, _ = clip.load(pretrained, "cuda").eval()
            checkpoint = self.clip_model.state_dict()
        else:
        ...
        if 'positional_embedding' in state_dict.keys():
            if self.positional_embedding.shape != state_dict['positional_embedding'].shape:
                ...
                spatial_pos = F.interpolate(
                    state_dict["positional_embedding"][1:, ].reshape(1, 16, 16, 1024).permute(0, 3, 1, 2),
                    size=(self.spatial_size, self.spatial_size), mode='bilinear')
                spatial_pos = spatial_pos.reshape(1024, self.spatial_size * self.spatial_size).permute(1, 0)
    ...
    def encode_text(self, text):
        ...
        if z.ndim == 2:
            z = z[:, None, :]
        z = einops.repeat(z, 'b 1 d -> b k d', k=1)
        return z
    def forward(self, x: torch.Tensor, with_pos = True):
        x = self.conv1(self.preprocess(x))  # shape = [*, width, grid, grid]
        B, C, H, W = x.shape
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
