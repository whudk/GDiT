...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, label=None, class_ids=None, mask=None):
        h = self.heads
        ...
        #             tmp_mask = F.interpolate(tmp_mask.unsqueeze(0), (H, W), mode="nearest")[0, 0, :, :]
...
class BasicTransformerBlock(nn.Module):
    ...
    #     return checkpoint(self._forward, (x, context, label, class_ids), self.parameters(), self.checkpoint)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x,  context=None, label=None, class_ids=None,graph  = None):
