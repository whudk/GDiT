...
class FrozenClipImageEmbedder(nn.Module):
    ...
    def forward(self, x, use_final):
        # x is assumed to be in range [-1,1]
        return self.model.encode_image(self.preprocess(x), use_final).float()

    def encode(self, im, mlp):
        if self.use_final:
            return mlp(self(im, self.use_final)).unsqueeze(1)
        else:
            return mlp(self(im, self.use_final))
